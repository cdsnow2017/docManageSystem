...
//		List<String> = new arryl
		findRoleById.stream().map(String::valueOf);
		Collectors.toList();
		return null;
	 ...
	 * token.getPrincipal();
	 * 
	 * // 第二步：根据用户输入的userCode从数据库查询 SysUser sysUser = null; try { sysUser =
	 * sysService.findSysUserByUserCode(userCode); } catch (Exception e1) { //
	 * TODO Auto-generated catch block e1.printStackTrace(); }
	 * 
	 * // 如果查询不到返回null if(sysUser==null){// return null; } // 从数据库查询到密码 String
	 * password = sysUser.getPassword();
	 * 
	 * //盐 String salt = sysUser.getSalt();
	 * 
	 * // 如果查询到返回认证信息AuthenticationInfo
	 * 
	 * //activeUser就是用户身份信息 ActiveUser activeUser = new ActiveUser();
	 ...
	 * 
	 * //根据用户id取出菜单 List<SysPermission> menus = null; try { //通过service取出菜单
	 * menus = sysService.findMenuListByUserId(sysUser.getId()); } catch
	 * (Exception e) { // TODO Auto-generated catch block e.printStackTrace(); }
	 * //将用户菜单 设置到activeUser activeUser.setMenus(menus);
	 * 
	 * //将activeUser设置simpleAuthenticationInfo SimpleAuthenticationInfo
	 * simpleAuthenticationInfo = new SimpleAuthenticationInfo( activeUser,
